...
class AristaRPCWrapper(object):
    ...
    def delete_vm_bulk(self, tenant_id, vm_id_list,
        ...
        for vm_id in vm_id_list:
            ...
            if self.cli_commands['baremetal']:
                ...
                # Delete the corresponding host as well
                if bm_host_list:
                    for h in bm_host_list:
                        if h['port_id'] == vm_id:
                            cmds.append('no host-id %s' % h['host_id'])
            else:
    ...
    def create_bm_hosts_bulk(self, bm_ports, bm_port_profiles, all_arista_nets,
        ...
        for bm_port in bm_ports:
            # hp = [p for p in bm_port_profiles
            #     if p.port_id in bm_port['id']]

            hp = None
            for p in bm_port_profiles:
                if p['port_id'] in bm_port['id']:
                    hp = p
                    break
            if hp:
                cmds = ['host-id %s' % hp['host_id']]
                profile = json.loads(hp['profile'])
                ...
                # Following is a temporary code for native VLANs-to be removed
                vlan_id = all_arista_nets[bm_port['network_id']]['segmentationTypeId']
                # remember to remove - as it will be created dynamically
                cmds.append('vlan %s' % vlan_id)
                cmds.append('interface %s' % l['port_id'])
                cmds.append('switchport trunk native vlan %s' % vlan_id)

                if hp['host_id'] in eos_hosts:
                    del eos_hosts[hp['host_id']]
            ...
            # Clean up all the stale baremetal hosts in EOS
            self.delete_stale_baremetal_hosts_from_eos(eos_hosts)

    def delete_stale_baremetal_hosts_from_eos(self, eos_hosts):
        # Now clean up any stale hosts in EOS
        if eos_hosts:
            cmds = []
            for host in eos_hosts.values():
                cmds.append('no host-id %s' % host['hostName'])
...
class SyncService(object):
    ...
    def synchronize(self):
            ...
            try:
                if vms_to_delete:
                    self._rpc.delete_vm_bulk(tenant, vms_to_delete,
                                             bm_host_list=bm_port_profiles, sync=True)
                if nets_to_delete:
        ...
        for tenant in vms_to_update:
            ...
            try:
                ...
                # in.
                ports_of_interest = [
                    port for port in neutron_ports if port['device_id'] in vms_to_update[tenant]
                ]

                # Create Baremetal hosts
                bm_ports = [
                    port for port in ports_of_interest
                    if port['device_owner'].startswith('baremetal')
                ]

                if bm_ports and bm_port_profiles:
                    self._rpc.create_bm_hosts_bulk(bm_ports, bm_port_profiles,
                                                   all_arista_nets, eos_bm_hosts,
                                                   sync=True)

                if ports_of_interest:
                    db_vms = db_lib.get_vms(tenant)
                    self._rpc.create_vm_port_bulk(tenant, ports_of_interest, db_vms,
                                                  sync=True)
            except arista_exc.AristaRpcError:
                LOG.warning(EOS_UNREACHABLE_MSG)
                self._force_sync = True
        ...
        if eos_bm_hosts:
            # Clean any stale baremetal hosts in EOS that are not needed anymore
            for hp in bm_port_profiles:
                if hp['host_id'] in eos_bm_hosts:
                    del eos_bm_hosts[hp['host_id']]
            self._rpc.delete_stale_baremetal_hosts_from_eos(eos_bm_hosts)
    ...
    def _get_eos_vms(self, eos_tenants, tenant):
        ...
        if eos_tenants and tenant in eos_tenants:
            ...
            if self._rpc.cli_commands['baremetal']:
