...
from neutron.plugins.ml2.drivers.arista import config
...
ARISTA_TYPE_DRIVER_OPTS = [
    cfg.IntOpt('vlan_sync_interval',
               default=10,
               help=_('VLAN Sync interval in seconds between Neutron plugin and '
                      'EOS. This interval defines how often the VLAN'
...
class AristaVlanTypeDriver(helpers.SegmentTypeDriver):
    ...
    def release_segment(self, session, segment):
        ...
        alloc_vlans = frozenset()
        if self.rpc.cli_commands['resource-pool']:
            eos_alloc_vlans = self.rpc.get_vlan_allocation()['allocatedVlans']
            alloc_vlans = frozenset(self.rpc.parse_vlan_ranges(eos_alloc_vlans))

        with session.begin(subtransactions=True):
            query = (session.query(VlanAllocation).
                     filter_by(physical_network=physical_network,
                               vlan_id=vlan_id))
            if vlan_id in alloc_vlans:
...
class VlanSyncService(object):
    """Periodically pull vlan allocation from CVX into the
    OpenStack db.
    """
