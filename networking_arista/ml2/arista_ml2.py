...
class AristaRPCWrapper(object):
    ...
    def check_cli_commands(self):
        ...
        cmd = ['show openstack resource-pool vlan region %s uuid' % self.region]
        try:
            self._run_eos_cmds(cmd)
            self.cli_commands['resource-pool'] = cmd
        except arista_exc.AristaRpcError:
            self.cli_commands['resource-pool'] = []
            LOG.warn(_LW("'resource-pool' command '%s' is not available on EOS"),
                     cmd)
...
class SyncService(object):
    ...
    def synchronize(self):
        ...
        try:
            ...
            LOG.warning(self._rpc.EOS_UNREACHABLE_MSG)
            self._force_sync = True
            return
