...
class AristaL3Driver(object):
    ...
    def _parse_protected_vlans(self, vlan_strings):
        ...
        for vlan_string in vlan_strings:
            if ':' in vlan_string:
                if vlan_string.count(':') != 1:
                    self._raise_invalid_protected_vlans(vlan_string)
                min_vlan, max_vlan = vlan_string.split(':')
                try:
                    min_vlan = int(min_vlan)
                except ValueError:
                    self._raise_invalid_protected_vlans(vlan_string)
                try:
                    max_vlan = int(max_vlan)
                except ValueError:
                    self._raise_invalid_protected_vlans(vlan_string)
                if (min_vlan > max_vlan
                        or min_vlan < const.MIN_VLAN_TAG
                        or min_vlan > const.MAX_VLAN_TAG
                        or max_vlan < const.MIN_VLAN_TAG
                        or max_vlan > const.MAX_VLAN_TAG):
                    self._raise_invalid_protected_vlans(vlan_string)
                ...
                try:
                    ...
                    self._raise_invalid_protected_vlans(vlan_string)
                if (vlan < const.MIN_VLAN_TAG
                        or vlan > const.MAX_VLAN_TAG):
                    self._raise_invalid_protected_vlans(vlan_string)
                vlans.add(vlan)
        return vlans
