...
class AristaRPCWrapper(object):
    ...
    def check_cli_commands(self):
        ...
        try:
            ...
            # Note the space before the sync token.
            self.cli_commands[CMD_REGION_SYNC] = ' sync'
        except arista_exc.AristaRpcError:
            self.cli_commands[CMD_REGION_SYNC] = ''
    ...
    def create_network_bulk(self, tenant_id, network_list, sync=False):
        ...
        for network in network_list:
            ...
            if(sync and self.cli_commands[CMD_REGION_SYNC] and
                    (counter % HEARTBEAT_INTERVAL) == 0):
                append_cmd('sync heartbeat')
    ...
    def delete_network_bulk(self, tenant_id, network_id_list, sync=False):
        ...
        for network_id in network_id_list:
            ...
            if(self.cli_commands[CMD_REGION_SYNC] and
                    (counter % HEARTBEAT_INTERVAL) == 0):
    ...
    def _build_command(self, cmds, sync=False):
        ...
        sync_token = self.cli_commands[CMD_REGION_SYNC] if sync else ''
        full_command = [
            'enable',
            'configure',
            'cvx',
            'service openstack',
            'region %s%s' % (self.region, sync_token),
        ]
