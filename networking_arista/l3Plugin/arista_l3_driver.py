...
class AristaL3Driver(object):
    ...
    def __init__(self):
        ...
        self._username = cfg.CONF.l3_arista.l3_host_username
        self._password = cfg.CONF.l3_arista.l3_host_password
        self._pri_l3_host = cfg.CONF.l3_arista.primary_l3_host
        self._sec_l3_host = cfg.CONF.l3_arista.secondary_l3_host
        self._l3_switches = cfg.CONF.l3_arista.l3_switches_uri
    ...
    def _set_eapi_client_for_switches(self):
        for switch in self._hosts:
            self._servers.append(self._make_eapi_client(
                switch.get('switch'), switch.get('user'),
                switch.get('password'), switch.get('scheme')))

    def _get_l3_switches_and_credentials(self):
        l3_switches_info = []
        for switch in self._l3_switches:
            parse_res = parse.urlparse(switch)
            l3_switches_info.append(
                {'switch': parse_res.hostname,
                 'user': parse_res.username if parse_res.username else
                 self._username,
                 'password': parse_res.password if parse_res.password else
                 self._password,
    ...
    @staticmethod
    def _make_eapi_client(host, username, password, scheme):
    ...
    def _validate_config(self):
        ...
        if self._pri_l3_host or self._sec_l3_host:
            msg = _("The primary_l3_host and secondary_l3_host parameters are "
                    "no longer supported. The new 'l3_switches_uri' parameter "
                    "should be used instead.")
            LOG.error(msg)
            raise arista_exc.AristaServicePluginConfigError(msg=msg)

        if not self._username:
            msg = _('Required option l3_host_username is not set.')
            LOG.error(msg)
            raise arista_exc.AristaServicePluginConfigError(msg=msg)
    ...
    def create_router_on_eos(self, router_name, rdm, server):
        ...
        cmds.extend(
            c.format(self._virtual_mac_addr)
            for c in self._additionalRouterCmdsDict['create']
            if (VIRTUAL_MAC_ADDR_CMD in c and self._virtual_mac_addr) or (
                VIRTUAL_MAC_ADDR_CMD not in c)
            )
