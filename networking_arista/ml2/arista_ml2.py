...
class Instance:
    ...
    VIRTUAL_INSTANCES = [DHCP, ROUTER, VM]
    BAREMETAL_INSTANCES = [BAREMETAL]
...
class AristaRPCWrapperJSON(AristaRPCWrapperBase):
    ...
    def _create_port_data(self, port_id, tenant_id, network_id, instance_id,
        ...
        vlan_type = 'allowed'
        if instance_type in Instance.BAREMETAL_INSTANCES:
            vlan_type = 'native'
    ...
    def create_instance_bulk(self, tenant_id, neutron_ports, vms,
        ...
        for vm in vms.values():
            ...
            for v_port in vm['ports']:
                ...
                if instance_type in Instance.VIRTUAL_INSTANCES:
                    portBinding = self._get_host_binding_dict(
                        port_id, inst_host, network_id,
                        networkSegments[network_id])
                elif instance_type in Instance.BAREMETAL_INSTANCES:
                    switch_profile = bm_port_profiles[neutron_port['id']]
                    switch_profile = json.loads(switch_profile['profile'])
    ...
    def delete_instance_bulk(self, tenant_id, instance_id_list, instance_type,
                             sync=False):
        path = 'region/%(region)s/%(type)s?tenant_id=%(tenant_id)' % {
               'region': self.region,
               'type': instance_type,
               'tenant_id': tenant_id}
    ...
    def delete_vm_bulk(self, tenant_id, vm_id_list, sync=False):
        self.delete_instance_bulk(tenant_id, vm_id_list, Instance.VM)

    def delete_dhcp_bulk(self, tenant_id, dhcp_id_list):
        self.delete_instance_bulk(tenant_id, dhcp_id_list, Instance.DHCP)
    ...
    def plug_port_into_network(self, device_id, host_id, port_id,
        ...
        device_type = ''
        if device_owner == n_const.DEVICE_OWNER_DHCP:
            device_type = Instance.DHCP
        elif device_owner.startswith('compute'):
            device_type = Instance.VM
        elif device_owner.startswith('baremetal'):
            device_type = Instance.BAREMETAL
        elif device_owner == n_const.DEVICE_OWNER_DVR_INTERFACE:
            device_type = Instance.ROUTER
        ...
        self._send_api_request('region/' + self.region + '/port', 'POST',
                               [port])
        if device_type in Instance.VIRTUAL_INSTANCES:
            self.bind_port_to_host(port_id, host_id, net_id, segments)
        elif device_type in Instance.BAREMETAL_INSTANCES:
    ...
    def unplug_port_from_network(self, device_id, device_owner, hostname,
        ...
        self.delete_port(port_id, device_id, tenant_id, device_type)
        port = self.get_port(tenant_id, port_id, device_id, device_type)
        if not port:
            path = 'region/%(region)s/%(type)s?tenantId=%(tenant_id)s' % {
                   'region': self.region,
                   'type': device_type,
                   'tenant_id': tenant_id,
            }
            data = [{'id': device_id}]
            self._send_api_request(path, 'DELETE', data)
