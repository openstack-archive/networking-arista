...
class AristaRPCWrapperBase(object):
    ...
    def create_port_bulk(self, tenant_id, neutron_ports, arista_ports,
        ...
        :param sync: This flags indicates that the region is being synced.
        """
        raise NotImplementedError("create_port_bulk() not implemented")
...
class AristaRPCWrapperJSON(AristaRPCWrapperBase):
    ...
    def _send_request(self, host, path, method, data=None):
        ...
        try:
            msg = (_('JSON request to %(url)s contains %(data)s') %
                   {'url': url, 'data': data})
            LOG.info(msg)
            func = None
            if method == 'GET':
    ...
    def check_cli_commands(self):
        # move this to register_with_eos?
        return self.create_region(self.region)
    ...
    def sync_start(self):
        ...
        region = self._send_api_request(path, 'GET')
    ...
    def sync_end(self):
        path = 'region/' + self.region
        region = self._send_api_request(path, 'GET')
        if region['requester'] != self.current_sync_name:
            return False
        data = {
            'name': self.region,
            'requester': '',
    ...
    def _create_instance(self, vm_id, host_id):
        return {
            'id': vm_id,
            'hostId': host_id
        }

    def _create_port(self, port_id, tenant_id, network_id, instance_id, name,
                     port_type):
        return {
            'id': port_id,
            'tenantId': tenant_id,
    ...
    def create_port_bulk(self, tenant_id, neutron_ports, arista_ports,
        ...
        for neutron_port in neutron_ports:
            try:
                vm = arista_ports[neutron_port['id']]
            except KeyError:
                LOG.warn(_LW(
            ...
            instance = self._create_instance(vm['deviceId'], vm['host'])

            device_owner = neutron_port['device_owner']
            if device_owner == n_const.DEVICE_OWNER_DHCP:
                instance_type = 'dhcp'
                dhcpInst.append(instance)
            elif device_owner.startswith('compute'):
                instance_type = 'vm'
                vmInst.append(instance)
            elif device_owner == n_const.DEVICE_OWNER_DVR_INTERFACE:
            ...
            port = self._create_port(neutron_port['id'], tenant_id,
                                     neutron_port['network_id'],
                                     vm['deviceId'], neutron_port.get('name'),
                                     instance_type)
            portInst.append(port)

        # create instances first
        if vmInst:
            path = 'region/' + self.region + '/vm'
            self._send_api_request(path, 'POST', vmInst)
        if dhcpInst:
            path = 'region/' + self.region + '/dhcp'
            self._send_api_request(path, 'POST', dhcpInst)
    ...
    def delete_port(self, port_id, instance_id, tenant_id, instance_type):
        ...
        port = self._create_port(port_id, tenant_id, None, instance_id, None,
                                 instance_type)
    ...
    def plug_host_into_network(self, vm_id, host, port_id,
                               network_id, tenant_id, port_name):
        vm = self._create_instance(vm_id, host)
        port = self._create_port(port_id, tenant_id, network_id, vm_id,
                                 port_name, 'vm')
    ...
    def plug_dhcp_port_into_network(self, dhcp_id, host, port_id,
                                    network_id, tenant_id, port_name):
        dhcp = self._create_instance(dhcp_id, host)
        port = self._create_port(port_id, tenant_id, network_id, dhcp_id,
                                 port_name, 'dhcp')
...
class AristaRPCWrapperCapi(AristaRPCWrapperBase):
    def __init__(self):
        super(AristaRPCWrapperCapi, self).__init__()
